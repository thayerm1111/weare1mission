...
export async function placeGenxGold(sig: { side: "buy" | "sell"; entryLow: number | null; entryHigh: number | null; stop: number | null; tp: number | null }): Promise<{ members: number; placed: number }> {
  ...
  for (const s of rows) {
    try {
      // Out of credits → paused. RE-CHECK through the meter instead of trusting the flag
      // (live case 09-02, single-account genx member: the flag only cleared on the FLOW
      // auto-run billing path, so a paused member who TOPPED BACK UP kept being skipped
      // here — silently — and missed two full desk fan-outs with 172 credits on balance).
      // meterAutoRun bills the 30-min window when due and clears the pause the moment
      // they can pay; only a member who STILL can't pay is skipped — and that skip is
      // now logged, never silent.
      if (s.credit_paused && !(await meterAutoRun(admin, s))) {
        try { await admin.from("flow_auto_events").insert({ user_id: s.user_id, symbol: "XAUUSD", status: "skipped", reason: "genx: credit_paused" }); } catch { /* log is best-effort */ }
        continue;
      }
