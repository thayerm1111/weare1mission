...
// ── MIRROR: one active trade per symbol, PER MEMBER ──────────────────────────
// A member who is already IN an open trade on a pair never stacks a second,
// divergent entry on that pair — they sit out until their trade closes. This was
// previously enforced DESK-WIDE (any one member's open position blocked every
// other account), which froze the whole desk for hours whenever one member held a
// long-running trade or a personal "play" on the symbol: on 08-27 two members'
// open gold sells blocked all other (flat) accounts from every GENX entry from
// 14:22 to past 23:30. Per-member keeps the anti-stacking protection where it
// matters while letting flat accounts take new signals. Same-pass placements
// (<5 min, so all accounts still enter the SAME signal together) are allowed.
const MIRROR_SAME_PASS_MS = 5 * 60_000;
async function userInActiveTrade(admin: Admin, userId: string, symbol: string): Promise<boolean> {
  try {
...
async function guardAndPlace(admin: Admin, settings: AutoSettings, ctx: GuardCtx, symbol: string, side: "buy" | "sell", levels: Levels, price?: number | null): Promise<SymbolResult> {
  ...
  // MIRROR (per member): if THIS member is already in an OPEN trade on this pair
  // from an earlier signal, they sit out — never a later, divergent entry on top.
  // Other (flat) members are unaffected.
  if (await userInActiveTrade(admin, settings.user_id, symbol)) return { symbol, action: "mirror_wait" };
...
export async function placeGenxGold(sig: { side: "buy" | "sell"; entryLow: number | null; entryHigh: number | null; stop: number | null; tp: number | null }): Promise<{ members: number; placed: number }> {
  ...
  // MIRROR is now PER MEMBER (checked inside the fan-out loop below): a member
  // already holding an open gold trade sits this ENTER NOW out; flat members take
  // it. The old desk-wide check here froze every account for hours whenever any
  // one member held a long-running gold trade or personal play.
  ...
  for (const s of rows) {
    try {
      ...
      // PER-MEMBER MIRROR: this member already holds an open gold trade from an
      // earlier signal → they sit out; flat members still take this entry.
      if (await userInActiveTrade(admin, s.user_id, "XAUUSD")) continue;
