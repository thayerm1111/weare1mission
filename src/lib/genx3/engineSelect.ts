...
export function selectBrain(version: string | null | undefined): "3.1.0" | "3.2.2" | null {
  if (version === "3.1.0" || version === "3.2.2") return version;
  // Rollout alias for the 3.2.1 → 3.2.2 upgrade (3.2.2 = 3.2.1 + the PDH/PDL break-retest module): lets the new
  // worker start while the control row still says 3.2.1, so the switchover needs no OFF window. Control is moved
  // to 3.2.2 right after deploy.
  if (version === "3.2.0" || version === "3.2.1") return "3.2.2";
  return null;
}
