/** Shared, causal trade simulator on 1m bars (live shadow tracking and research replays). */
...
export type SimOut = { filled: boolean; open?: boolean; missReason?: "expired" | "invalidated_before_fill"; fillAt?: number; fill?: number; exitAt?: number; exit?: number; result?: "target" | "stop" | "timeout"; pnl?: number; r?: number; mfe?: number; mae?: number; holdMin?: number };
...
export function simulate(all: B[], p: SimIn): SimOut {
  ...
  return { filled: true, open: true, fillAt, fill, mfe: +mfe.toFixed(2), mae: +mae.toFixed(2) };   // bars ran out while the position is still open
}
