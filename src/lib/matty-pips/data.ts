...
export type MarketRead = {
  ...
  /** 5M trigger timeframe — OPTIONAL: empty array when the feed is busy, and
   *  every consumer fails soft to a neutral micro read. Never blocks the read. */
  m5: Candle[];
  price: number;
};
...
/** Full top-down pull: Daily, 4H, 1H, 15M, 5M + live price (sanity-checked).
 *  The 5M series is the TRIGGER timeframe and is best-effort: a rate-limited
 *  or short 5M pull never fails the read — it comes back empty and the micro
 *  engine goes neutral. The four core timeframes keep their hard requirements. */
export async function fetchMarket(canonical: string, mdKey: string, fresh: boolean): Promise<MarketRead | MarketReadError> {
  ...
  return { ok: true, d, h4, h1, m15, m5, price };
}
