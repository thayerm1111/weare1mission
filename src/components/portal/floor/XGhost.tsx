...
export function XGhost() {
  ...
  const run = useCallback(async () => {
    ...
    try {
      ...
      if (r.status === 402 || d.error === "insufficient_credits") { setMsg("You're out of credits — a scan costs 5. Free credits reset weekly, or top up on the Credits page."); try { window.dispatchEvent(new Event("open-credits-flyer")); } catch { /* ignore */ } return; }
