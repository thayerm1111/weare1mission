...
test('size comes from risk, and rounds DOWN', () => {
  /*
   * Risk sizing in isolation: money-at-risk divided by stop distance and value per pip, rounded DOWN.
   * Position-size ceilings are off by default (the owner's model is risk percentage alone), so this
   * runs exactly as shipped; the optional ceilings have their own file, cc-position-cap.test.ts.
   */
  ...
  assert.ok(r.ok && r.cappedBy == null, 'nothing bounds size but the risk setting itself');

  // And a ceiling, when a caller asks for one, still applies.
  const capped = sizePosition({
    equity: 10_000, entry: 4350, stop: 4345, side: 'buy', riskPct: 0.5, inst, maxNotionalXEquity: 2,
  });
  assert.ok(capped.ok && capped.lots < 0.1, 'a $10k account capped at 2x does not control $435k of gold');
});
